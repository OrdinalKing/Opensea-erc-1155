...
contract Token is Ownable, ERC1155 {
    ...
    constructor() ERC1155("https://gateway.pinata.cloud/ipfs/") {
        setContractURI("https://gateway.pinata.cloud/ipfs/");
    }
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        require(exists[tokenId], "ERC721Metadata: URI query for nonexistent token");
            ...
            if (bytes(_tokenURI).length > 0) {
                return string(abi.encodePacked(_contractURI, _tokenURI));
            }
    ...
    function mintBatch(address to, string[] memory uris, uint256[] memory amounts) public onlyOwner{
        uint256[] memory ids = new uint256[](uris.length);

        for(uint256 i=0 ; i<uris.length ; i++) {
            _tokenIds.increment();
            uint256 id = _tokenIds.current();
            _mint(to, id, amounts[i], "");
            _tokenURIs[id] = uris[i];
            exists[id] = true;
            _tokenSupply[id] = _tokenSupply[id].add(amounts[i]);
        }
